...
const findOrCreateManagedIncomingLead = async (
  ...
  if (normalizedPhone) {
    lead = await Lead.findOne({ phone: { $regex: `${normalizedPhone}$` } }).exec()
  }
  if (!lead && normalizedPhone) {
    lead = await Lead.findOne({ phone: normalizedPhone }).exec()
  }
  if (!lead && rawPhoneNumber) {
    lead = await Lead.findOne({ phone: rawPhoneNumber }).exec()
  }
  ...
  if ((!lead.name || /^Lead\s+\d{4}$/i.test(lead.name) || /^Exotel\s+\d{4}$/i.test(lead.name)) && (normalizedPhone || rawPhoneNumber)) {
...
const findOrCreateExotelLead = async (
  ...
  if (isIncoming && lead.source !== 'Direct') {
    lead.source = 'Direct'
  }
  if ((!lead.name || /^Lead\s+\d{4}$/i.test(lead.name) || /^Exotel\s+\d{4}$/i.test(lead.name)) && (normalizedPhone || rawPhoneNumber)) {
    lead.name = buildExotelLeadName(normalizedPhone || rawPhoneNumber)
...
const processManagedCallback = async (payload: ExotelV3StatusCallbackPayload): Promise<void> => {
  ...
  if (!call && direction === 'incoming') {
    ...
    if (leadResult?.lead && manager) {
      call = await Call.create({
        lead: leadResult.lead._id,
        leadName: leadResult.lead.name,
        phone: leadResult.lead.phone,
        representative: manager._id,
        representativeName: manager.name,
        exophoneNumber,
        exotelCallSid: callSid,
