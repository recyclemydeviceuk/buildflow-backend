...
const ensureLeadForDialerPhone = async (params: {
  ...
  // Search both primary and alternate phone so dialling an alternate number
  // doesn't create a second lead record
  let lead =
    (await Lead.findOne({ $or: [{ phone: { $regex: `${normalizedPhone}$` } }, { alternatePhone: { $regex: `${normalizedPhone}$` } }] }).exec()) ||
    (await Lead.findOne({ $or: [{ phone: normalizedPhone }, { alternatePhone: normalizedPhone }] }).exec()) ||
    (await Lead.findOne({ $or: [{ phone: params.phone }, { alternatePhone: params.phone }] }).exec())
...
export const initiateCall = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { leadId, phone, leadName, city, agentPhone, representativeId, recordCall, useAlternatePhone } = req.body
    ...
    const phoneToCall = useAlternatePhone && lead.alternatePhone ? lead.alternatePhone : lead.phone
    const destinationPhone = toDialablePhone(phoneToCall)
    if (!destinationPhone) {
      return res.status(400).json({ success: false, message: useAlternatePhone ? 'Alternate phone number is not dialable' : 'Lead phone number is not dialable' })
    }
...
export const postCallFeedback = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (normalizedDisposition) {
      leadUpdatePayload.disposition = normalizedDisposition
    }
    // Append a status note when disposition changes so the change appears in audit trail
    const leadUpdateOp: Record<string, unknown> = { $set: leadUpdatePayload }
    if (normalizedDisposition && notes) {
      leadUpdateOp.$push = {
        statusNotes: {
          status: normalizedDisposition,
          note: notes,
          createdAt: new Date(),
    ...
    const updatedLead = await Lead.findByIdAndUpdate(call.lead, leadUpdateOp, { new: true })
